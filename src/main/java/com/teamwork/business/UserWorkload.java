 ...
 * - Dùng để hiển thị Thẻ Hồ Sơ Đồng Đội (Social Profile Card)
...
public class UserWorkload implements Serializable {
    ...
    private List<Integer> relatedTaskIds;    // Danh sách ID các Task mà người này có tham gia (Lead hoặc làm con)
    ...
    public UserWorkload(User user, int leadTaskCount, int subTaskCount, int completedSubTaskCount, List<Integer> relatedTaskIds) {
        this.user = user;
    ...
    /**
     * Tính tổng số lượng đầu việc mà thành viên này đang gánh (Task Lead + Việc Con)
     */
    ...
    /**
     * Chuyển danh sách ID các Task liên quan [1, 3, 5] thành chuỗi "1,3,5"
     * để gắn vào thuộc tính data-related-tasks của HTML trên giao diện JSP.
     */
